...
extension SocialLoginViewModel: ASAuthorizationControllerDelegate {
    ...
    func startSignInWithAppleFlow() {
        ...
        authorizationController.presentationContextProvider =
            UIApplication.shared.windows.last?.rootViewController as? ASAuthorizationControllerPresentationContextProviding
    ...
    public func authorizationController(controller _: ASAuthorizationController, didCompleteWithAuthorization authorization: ASAuthorization) {
        if let appleIDCredential = authorization.credential as? ASAuthorizationAppleIDCredential {
            ...
            guard let appleIDToken = appleIDCredential.identityToken else {
                ...
                if let c = completion {
                    c(nil, "Unable to fetch identity token")
                    completion = nil
                }
                return
            }
            guard let idTokenString = String(data: appleIDToken, encoding: .utf8) else {
                ...
                if let c = completion {
                    c(nil, "Unable to serialize token string from data: \(appleIDToken.debugDescription)")
                    completion = nil
                }
                return
            }
            ...
            if let fullName = appleIDCredential.fullName, let givenName = fullName.givenName, let familyName = fullName.familyName {
                let displayName = "\(givenName) \(familyName)"
                submitCredential(credential: credential, fullName: displayName)
            } else {
